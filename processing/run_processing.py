...
async def run_processing():
    ...
    async with VectorStore() as vector_store:
        ...
        logger.info(
            f"Studies saved to cloud SQL | saved = {studies_saved} | total = {len(studies)}")

        logger.info("Chnunking studies...")
        all_chunks = chunker.chunk_studies(studies)
        logger.info(f"Total Chunks created: {len(all_chunks)}")

        logger.info("Embedding Chunks via Jina Embeddings")
        embedded_chunks = await embedder.embed_chunks(all_chunks)
        logger.info(f"Total Chunks Embedded: {len(embedded_chunks)}")

        logger.info(f"Saving embedded chunks to Cloud SQL")
