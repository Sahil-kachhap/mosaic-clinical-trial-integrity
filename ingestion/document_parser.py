...
class DocumentParser:
    def parse_study(self, raw: dict[str, Any]) -> ParsedStudy | None:
        try:
            ...
            if not nct_id:
                logger.warning("Study is missing is its NCT ID - skipping")
                return None
            ...
            nct_id = raw.get("protcolSection", {}).get(
                "identificationModule", {}).get("nctId", "UNKNOWN")
            logger.error(
    ...
    def parse_studies(self, raw_studies: list[dict[str, Any]]) -> list[ParsedStudy]:
        ...
        logger.info(
            f"Parsed Studies | Success = {len(parsed)} | Failed = {failed} | Total = {len(raw_studies)}")
        return parsed
    ...
    def parse_paper(self, raw: dict[str, Any]) -> ParsedPaper | None:
        try:
            ...
            logger.error(
                f"Failed to parse paper | pmid = {raw.get("pmid", "UNKNOWN")} | error = {e}")
            return None
    ...
    def parse_papers(self, raw_papers: list[dict[str, Any]])->list[ParsedPaper]:
        ...
        logger.info(f"Parsed Papers | success = {len(parsed)} | failed = {failed} | total = {len(raw_papers)}")
        return parsed
