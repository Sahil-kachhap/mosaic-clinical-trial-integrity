...
_llm = ChatGroq(model=settings.chat_model,
                temperature=0.1).bind_tools(AGENT_TOOLS)
...
async def missing_results_node(state: MosaicState) -> dict:
    ...
    try:
        ...
        system_prompt = f"""You are the Missing Results Agent for MOSAIC — a clinical trial research integrity intelligence system.
        YOUR MISSION:
        Find completed clinical trials that have never posted their results to ClinicalTrials.gov, violating federal law (FDAAA 801).
        By law, results must be posted within 12 months of primary completion.

        YOUR REASONING RULES (follow these exactly): {procedures_text}
        ...
        Begin your investigation now. Use your tools to search for completed studies with missing results. 
        Generate signals for every violation you find.
        """
        ...
        for iteration in range(max_iterations):
            ...
            if not response.tool_calls:
                logger.info(
                    f"Analysis complete | {AGENT_NAME} | iteration={iteration+1}")
        ...
        await _episodic.save_episodes(agent_name=AGENT_NAME, content=episode_content, outcome="signal_generated" if signals_found else "no_signal")
        logger.info(
